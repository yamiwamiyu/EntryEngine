...
namespace PSDFile.Text
{
    public class TdTaParser
    {
        private static readonly Regex arrayNotation = new Regex("^\\[(?<index>\\d+)\\]", RegexOptions.CultureInvariant);
        private static readonly Regex dotNotation = new Regex("^\\.?(?<key>[^\\.\\[\\]\\@\\$]+)", RegexOptions.CultureInvariant);
		...
		public static Color getColor(object tree, string selector)
		{
            ...
            byte r, g, b, a;
            a = (byte)((double)values[0] * 255);
            r = (byte)((double)values[1] * 255);
            g = (byte)((double)values[2] * 255);
            b = (byte)((double)values[3] * 255);
