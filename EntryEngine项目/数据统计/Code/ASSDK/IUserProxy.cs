...
class IUserProxy
{
    ...
    /// <see cref="IUser.Login(string, string, string, System.Action{int})"></see>
    public AsyncData<string> Login(string gameName, string deviceID, string channel, System.Action<int> callback)
    {
        var parameters = new StringBuilder();
        parameters.Append("gameName={0}&", gameName);
        parameters.Append("deviceID={0}&", deviceID);
        parameters.Append("channel={0}", channel);
        return send("3/Login", parameters.ToString(), callback);
    }
    
    /// <see cref="IUser.Online(int, System.Action{bool})"></see>
    public AsyncData<string> Online(int loginID, System.Action<bool> callback)
    {
        var parameters = new StringBuilder();
        parameters.Append("loginID={0}", loginID);
        return send("3/Online", parameters.ToString(), callback);
    }
    
    /// <see cref="IUser.Analysis(int, System.Collections.Generic.List{T_Analysis}, System.Action{bool})"></see>
    public AsyncData<string> Analysis(int loginID, System.Collections.Generic.List<T_Analysis> analysis, System.Action<bool> callback)
    {
        var parameters = new StringBuilder();
        parameters.Append("loginID={0}&", loginID);
        parameters.Append("analysis={0}", JsonWriter.Serialize(analysis));
        return send("3/Analysis", parameters.ToString(), callback);
    }
