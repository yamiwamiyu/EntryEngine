...
namespace EntryBuilder.CSharpShell
{
...
namespace EntryBuilder.JSShell
{
    abstract class Shell
    {
        public abstract void Protect(ref byte[] bytes);
    ...
    class ShellReverse : Shell
    {
        ...
        public override void BuildLoadCode(StringBuilder builder)
        {
            builder.AppendLine("bytes.reverse();");
        }
    }
    class ShellPlus : Shell
    {
        ...
        public override void BuildLoadCode(StringBuilder builder)
        {
            builder.AppendLine("for (var i = 0; i < bytes.Length; i++)");
            builder.AppendBlock(() =>
            {
                builder.AppendLine("bytes[i] = (bytes[i] - {0}) & 255;", plus);
            });
        }
    }
    class ShellShuffle : Shell
    {
        ...
        public override void BuildLoadCode(StringBuilder builder)
        {
            builder.AppendLine("var indexes = [{0}];", string.Join(",", indexes.Select(i => i.ToString()).ToArray()));
            builder.AppendLine("var temp;");
            builder.AppendLine("for (var i = 0; i < indexes.Length; i++)");
            builder.AppendBlock(() =>
            {
                builder.AppendLine("var index = indexes[i];");
                builder.AppendLine("temp = bytes[index];");
                builder.AppendLine("bytes[index] = bytes[i];");
                builder.AppendLine("bytes[i] = temp;");
            });
