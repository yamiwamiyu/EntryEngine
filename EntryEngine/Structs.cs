...
namespace EntryEngine
{
    [AReflexible]public struct VECTOR2 : IEquatable<VECTOR2>
    {
        ...
        {
            float num2 = 1f / (float)Math.Sqrt(LengthSquared());
            this.X *= num2;
            this.Y *= num2;
        }
    ...
    public struct MATRIX : IEquatable<MATRIX>
    {
        ...
        //public static EEMatrix CreateFromQuaternion(Quaternion quaternion)
        //{
        ...
        //    return result;
        //}
        //public static void CreateFromQuaternion(ref Quaternion quaternion, out EEMatrix result)
        //{
        //    float num = quaternion.X * quaternion.X;
        //    float num2 = quaternion.Y * quaternion.Y;
        //    float num3 = quaternion.Z * quaternion.Z;
        ...
        //    result.M44 = 1f;
        //}
        //public static EEMatrix CreateFromYawPitchRoll(float yaw, float pitch, float roll)
        //{
        //    Quaternion quaternion;
        //    Quaternion.CreateFromYawPitchRoll(yaw, pitch, roll, out quaternion);
        //    EEMatrix result;
        //    EEMatrix.CreateFromQuaternion(ref quaternion, out result);
        //    return result;
        //}
        //public static void CreateFromYawPitchRoll(float yaw, float pitch, float roll, out EEMatrix result)
        //{
        //    Quaternion quaternion;
        //    Quaternion.CreateFromYawPitchRoll(yaw, pitch, roll, out quaternion);
        //    EEMatrix.CreateFromQuaternion(ref quaternion, out result);
    ...
    public abstract class Graph<T> : IEnumerable<T> where T : Graph<T>
    {
        ...
        public static Dictionary<T, GraphPath<T>> GPS(T start, int breadth, Func<T, bool> visitable, Func<GraphPath<T>, bool> onNewNode)
        {
            ...
            Dictionary<GraphPath<T>, IEnumerable<T>> nodes = new Dictionary<GraphPath<T>, IEnumerable<T>>();
            nodes.Add(path[start], start);
            GraphPath<T> temp;
            while (nodes.Count > 0)
            {
                d++;
                ...
                var next = new Dictionary<GraphPath<T>, IEnumerable<T>>();
                foreach (var road in nodes)
                {
                    foreach (var node in road.Value)
                    {
                        ...
                        {
                            if (path.TryGetValue(node, out temp))
                            {
                                if (d == temp.Distance)
                                {
                                    temp.Previous.Add(road.Key);
                                    var nextNodes = node.Where(n => !temp.Previous.Any(p => p.Node == n)).ToList();
                            ...
                            {
                                temp = new GraphPath<T>(node, d);
                                temp.Previous.Add(road.Key);
                                path.Add(node, temp);
                                if (onNewNode != null && onNewNode(temp))
                                    find = true;
                                next.Add(temp, node);
                            }
                        }
