...
namespace DragonBones
{
    ...
    public class BinaryDataParser : ObjectDataParser
    {
        ...
        private TimelineData _ParseBinaryTimeline(TimelineType type, uint offset)
        {
            return _ParseBinaryTimeline(type, offset, null);
        }
        private TimelineData _ParseBinaryTimeline(TimelineType type, uint offset, TimelineData timelineData)
        {
        ...
        public static Dictionary<string, object> DeserializeBinaryJsonData(byte[] bytes, out int headerLength, BinaryDataParser.JsonParseDelegate jsonParse)
        {
            headerLength = 0;
            ...
            using (BinaryDataReader reader = new BinaryDataReader(ms))
            {
                ...
                result = jsonParse != null ? jsonParse(headerString) as Dictionary<string, object> : new EntryEngine.Serialize.JsonReader(headerString).ReadDictionary();
